#!/usr/bin/env python3
"""Inactive cards: peek patches while hover/press, then collapse."""
...
KEEP_OLD = """      if(active&&e===active){if(d.open)nKeep++;return;}"""
KEEP_NEW = """      if((active&&e===active)||(window.patchPeekEntry&&e===window.patchPeekEntry)){if(d.open)nKeep++;return;}"""
...
REMEMBER_NEW = """function isActivePatchCard(entry){
...
TOGGLE_NEW = """document.addEventListener('pointerdown',function(e){
  var sum=e.target.closest&&e.target.closest('summary');
  if(!sum)return;
  var details=sum.parentNode;
