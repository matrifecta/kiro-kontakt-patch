...
THUMBS="$ART_DIR/catalog-thumbs-$MODE"; MANUAL="$ART_DIR/covers-manual"; OVERRIDES="$ART_DIR/cover-overrides.tsv"
IDX="$ART_DIR/.idx.$MODE.tmp"; BODY="$ART_DIR/.body.$MODE.tmp"
HOME_LIB="$HOME/.config/DecentSampler/Sample Libraries"
BTRFS="/mnt/btrfs_disk/DS Libraries"; WDB="/mnt/wd_black/DS Libraries"
mkdir -p "$ART_DIR" "$MANUAL"
...
emit_patches(){ local libdir="$1"
  ...
  printf '  <details class="patches" data-patch-count="%s"><summary>%s patch(es)</summary>\n' "$count" "$count"
...
entry(){ local name="$1" libdir="$2" openp="$3"
  ...
  local pnames; pnames=$(patch_names "$libdir")
  local kw; kw=$(classify "$name $d")   # name + desc feed instrument tags; patch tokens are teal
  ...
  local pcount=0
  pcount=$(find "$libdir" -maxdepth 4 -iname '*.dspreset' 2>/dev/null | grep -viE '/__MACOSX/|/\._' | grep -c .)
  [ -z "$pcount" ] && pcount=0
  {
    printf '<div class="entry" id="%s" data-kw="%s" data-gear="%s" data-name="%s" data-other="%s" data-patch="%s" data-patch-count="%s">\n' "$id" "$(e "$allkw")" "$(e "$gkw")" "$(eattr "$name")" "$(e "$other")" "$(e "$ptoks")" "$pcount"
...
begin_loc(){ printf '<div class="loc-group">\n' >> "$BODY"; }
end_loc(){ printf '</div>\n' >> "$BODY"; }
 ...
 *{box-sizing:border-box}
 ...
 .filter-wrap{position:sticky;top:0;z-index:210;background:var(--bg-surface);border-bottom:1px solid var(--border);box-sizing:border-box;width:100%;max-width:100vw;padding-left:env(safe-area-inset-left);padding-right:env(safe-area-inset-right);display:flex;flex-direction:column}
 ...
 .layout-presets-save button{flex:0 0 auto;min-height:2.25rem;padding:0 10px;border:1px solid var(--border);border-radius:6px;background:var(--bg-card);color:var(--text);font:inherit;cursor:pointer}
 ...
 .index{column-width:clamp(14rem,28vw,18rem);column-gap:clamp(1rem,2.5vw,2rem);font-size:.9rem;padding-left:1.2rem}
...
cat <<'HTML'
 ...
 .card-yt-list{display:flex;flex-direction:column;min-height:0}
 .card-yt-row{display:flex;align-items:center;gap:10px;width:100%;box-sizing:border-box;margin:0;padding:8px 10px;border:0;border-bottom:1px solid var(--border);background:transparent;color:var(--text);font:inherit;cursor:pointer;text-align:left;touch-action:manipulation}
 .card-yt-row.is-active{background:var(--accent-instrument-bg)}
 .card-yt-row img{width:4.5rem;height:2.5rem;object-fit:cover;border-radius:4px;flex:0 0 auto;background:var(--bg)}
 .card-yt-row-title{min-width:0;font-size:.92em;line-height:1.3;color:var(--text);overflow-wrap:anywhere}
 ...
 .card-search-stage.yt-wide .card-search-frame{position:relative;inset:auto;flex:1 1 58%;width:auto;height:100%;max-height:none;min-height:0}
 .card-search-stage.yt-wide .card-yt-comments{display:flex;flex:1 1 42%;min-width:11rem;border-top:0;border-left:1px solid var(--border)}
 ...
 body.search-mode .search-chrome{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);grid-template-rows:none;gap:8px;align-items:start;justify-content:stretch;position:sticky;top:0;z-index:200;background:var(--bg-surface);padding-top:env(safe-area-inset-top);padding-left:env(safe-area-inset-left);padding-right:env(safe-area-inset-right);padding-bottom:10px;width:100%;max-width:100%;box-sizing:border-box}
 body.search-mode:not(.kw-open) .search-chrome{position:sticky;top:0;z-index:200}
 body.search-mode .search-col{display:flex;flex-direction:column;min-width:0;max-width:100%;justify-self:start}
 body.search-mode .filter-wrap{position:relative;top:auto;z-index:auto;width:100%;max-width:100%;padding-left:0;padding-right:0;justify-self:end}
 ...
 body.search-mode:not(.kw-open):not(.search-chrome-collapsed) .search-chrome{grid-template-columns:minmax(0,1fr) 0 max-content}
 body.search-mode.kw-open.search-chrome-collapsed .search-chrome{grid-template-columns:max-content 0 minmax(0,1fr)}
 body.search-mode .search-col,body.search-mode .filter-wrap{width:auto;max-width:100%;min-width:0}
 body.search-mode.menus-collapsed .search-chrome{grid-template-columns:max-content minmax(0,1fr) max-content;justify-content:stretch;align-items:start;gap:0}
 body.search-mode.menus-collapsed .search-col{width:auto;flex:0 0 auto;justify-self:start}
 body.search-mode.menus-collapsed .filter-wrap{width:auto;flex:0 0 auto;justify-self:end}
 ...
 body.search-mode .filter-wrap:not(.open) .filter-toggle{border:none;background:transparent;box-shadow:none;width:auto;flex:0 0 auto;padding:.5rem .25rem;color:var(--text-muted);font-weight:600;justify-content:flex-end;text-align:right}
 body.search-mode .filter-wrap:not(.open) .filter-toggle .toggle-arrow{display:none}
 body.search-mode .filter-wrap:not(.open) .filter-top{justify-content:flex-end;border:none;padding:0}
 body.search-mode .filter-wrap:not(.open){width:auto;max-width:100%;min-width:0;justify-self:end}
 ...
 .search-ac-shell.ac-fs .ac-fs-bar{display:flex;align-items:center;gap:.5rem;flex-wrap:nowrap}
 ...
 body.search-mode .search-chrome.search-height-set{overflow:visible;align-items:stretch}
 ...
 body.layout-edit.search-mode .search-chrome.search-height-set{overflow:visible}
 ...
 .ac-fs-bar{display:none;flex:0 0 auto;align-items:center;gap:.5rem;padding:.5rem .75rem;padding-top:max(.5rem,env(safe-area-inset-top,0px));padding-left:max(.75rem,env(safe-area-inset-left,0px));padding-right:max(.75rem,env(safe-area-inset-right,0px));border-bottom:1px solid var(--border);background:var(--bg-surface);z-index:2;box-sizing:border-box}
 .search-ac-shell.ac-fs{position:fixed;z-index:280;display:flex;flex-direction:column;background:var(--bg-surface);overflow:hidden;box-shadow:none;max-width:none}
 .search-ac-shell.ac-fs .ac-fs-bar{display:flex}
 ...
 body.ac-fs-open{overflow:hidden}
 body.ac-fs-open .search-strip-hide{display:none!important}
 body.ac-fs-open .search-ac-shell.ac-fs .search-strip{display:flex;flex:0 0 auto;width:100%;max-width:100%;border-bottom:1px solid var(--border);background:var(--bg-surface);flex-wrap:nowrap}
 body.ac-fs-open .search-ac-shell.ac-fs .search-strip input{flex:1 1 auto;min-width:0}
 ...
 .clear-miss-btn,.layout-edit-btn,.layout-presets-btn,.ui-scale-step,.ui-scale-readout{min-height:2.75rem}
...
echo 'var CATALOG_NS="ds";window.CATALOG_NS="ds";';
cat <<'JS'
...
function cardSearchYtPlayUrl(id,host){
  ...
  var base=host==='youtube'?'https://www.youtube.com/embed/':'https://www.youtube-nocookie.com/embed/';
...
function cardSearchYtTryNext(reason){
  ...
  for(var i=0;i<items.length;i++){
    ...
    cardSearchState._ytHost='nocookie';
    if(setCardYtFrame(it.id,'nocookie')){
...
function renderCardYtList(items,activeId){
  ...
  if(!items||!items.length){
    list.innerHTML='<p>No embeddable videos for this library. Open YouTube in a popup.</p><button type="button" onclick="event.preventDefault();event.stopPropagation();cardSearchOpenPopup()">Open in popup</button>';
    return;
  }
...
function loadCardYtSearch(query){
  ...
  function finish(items){
    ...
    if(!setCardYtFrame(pick.id,'nocookie'))cardSearchYtTryNext('bad id');
...
function cardSearchShowStatus(msg,showPopup){
  ...
  if(showPopup!==false)html+='<button type="button" onclick="event.preventDefault();event.stopPropagation();cardSearchOpenPopup()">Open in popup</button>';
  html+='</div>';
...
function loadCardWebSearch(query){
  ...
  function merge(into, extraData){
    extraData=extraData||{};
    into.RelatedTopics=(into.RelatedTopics||[]).concat(extraData.RelatedTopics||[]);
    into.Results=(into.Results||[]).concat(extraData.Results||[]);
    if(!into.Abstract&&extraData.Abstract){
      into.Abstract=extraData.Abstract;
      into.AbstractURL=extraData.AbstractURL;
      into.Heading=extraData.Heading;
    }
    return into;
  }
  function apply(data){cardSearchState._webData=data||{};cardSearchRenderWeb(data||{});}
  var jobs=[];
  variants.slice(0,3).forEach(function(v,i){
    jobs.push(fetchCardWiki(v).then(wikiSearchToTopics).catch(function(){return {};}));
    if(i<2){
      jobs.push(fetchCardDdg(v).catch(function(){return {};}));
      jobs.push(fetchCardArchive(v).catch(function(){return {};}));
    }
  });
  Promise.all(jobs).then(function(parts){
    var data={RelatedTopics:[],Results:[],Abstract:'',Heading:''};
    parts.forEach(function(p){merge(data,p);});
    apply(data);
  }).catch(function(){
    cardSearchShowStatus('Web search could not be loaded here. Back returns to the card. Open the full search in a popup if you want more results.');
  });
}
...
function loadCardImageSearch(query){
  ...
  function pagesToItems(data){
    ...
    Object.keys(pages).forEach(function(k){
      ...
      if(!ii||!ii.url)return;
      if(ii.mime&&ii.mime.indexOf('image/')!==0)return;
      items.push({url:ii.url,thumb:ii.thumburl||ii.url,title:p.title||''});
    });
    return items;
  }
  function fetchCommons(term){
    var url='https://commons.wikimedia.org/w/api.php?action=query&format=json&origin=*&generator=search&gsrnamespace=6&gsrlimit=24&gsrsearch='+encodeURIComponent(term)+'&prop=imageinfo&iiprop=url|mime|size&iiurlwidth=640';
    return fetch(url,{mode:'cors'}).then(function(r){
      if(!r.ok)throw new Error('HTTP '+r.status);
      return r.json();
    }).then(pagesToItems);
  }
  function fetchWikiImages(term){
    var url='https://en.wikipedia.org/w/api.php?action=query&format=json&origin=*&generator=search&gsrsearch='+encodeURIComponent(term)+'&gsrlimit=12&prop=pageimages&piprop=thumbnail&pithumbsize=640';
    return fetch(url,{mode:'cors'}).then(function(r){
      if(!r.ok)throw new Error('HTTP '+r.status);
      return r.json();
    }).then(function(data){
      ...
      Object.keys(pages).forEach(function(k){
        var p=pages[k];
        var th=p&&p.thumbnail&&p.thumbnail.source;
        if(!th)return;
        items.push({url:th,thumb:th,title:p.title||''});
      });
      return items;
    });
  }
  function dedupe(items){
    var seen={},out=[];
    (items||[]).forEach(function(it){
      var key=(it.url||it.thumb||'')+'|'+(it.title||'');
  ...
  function tryAt(i, acc){
    if(i>=variants.length){finish(acc);return;}
    Promise.all([
      fetchCommons(variants[i]).catch(function(){return [];}),
      fetchWikiImages(variants[i]).catch(function(){return [];})
    ]).then(function(pair){
      var next=dedupe(acc.concat(pair[0]||[]).concat(pair[1]||[]));
      if(next.length>=8)finish(next);
      else tryAt(i+1,next);
    }).catch(function(){tryAt(i+1,acc);});
  }
  tryAt(0,[]);
}
...
function openCardSearchEmbed(type,popupUrl){
  ...
  if(type==='yt'){
    ...
    if(known)setCardYtFrame(known,'nocookie');
...
window.syncCardSearchTheme=function(){
  ...
  if(cardSearchState.type==='yt'){
    var next=cardSearchYtPlayUrl(cardSearchState.ytId||cardSearchYtIdFromUrl(cardSearchState.popupUrl));
    if(!next||next===cardSearchState.embedUrl)return;
...
function fillYtCommentsStrip(){
  if(cardSearchState.ytItems&&cardSearchState.ytItems.length)renderCardYtList(cardSearchState.ytItems,cardSearchState.ytId);
}
...
function ms(el){return sel.length>0&&sel.every(function(k){return pillMatch(el,k);});}
...
function kwTone(k,asPatch){if(asPatch||isPatchKw(k))return ' patch';if(GEAR[k])return ' gear';return '';}
...
entries.forEach(function(el){var catOk=catMatch(el);var kwMatch=ms(el);var show=catOk&&(sel.length===0||kwMatch);el.classList.toggle('hit',sel.length>0&&kwMatch&&catOk);el.classList.toggle('dim',!show);el.classList.toggle('is-hidden',!show);el.classList.remove('fav-rec');el.style.display=show?'':'none';});
...
function applySplit(){
  ...
  if(!bothOpen){
    ...
    if(searchCollapsed&&!kwOpen){
      ...
      el.style.gridTemplateColumns='max-content minmax(0,1fr) max-content';
      return;
    }
    if(searchCollapsed&&kwOpen){
      el.style.gridTemplateColumns='max-content 0px minmax(0,1fr)';
      return;
    }
    // Search alone may occupy full top chrome width; Keywords control stays top-right.
    el.style.gridTemplateColumns='minmax(0,1fr) 0px max-content';
    return;
  }
  ...
  // Soft on-screen mins only — saved ratio takes precedence.
  var soft=Math.min(4*rem(),Math.max(0,avail*0.05));
  var mins={a:soft,b:soft};
...
function parkSearchStrip(){
  ...
  if(acFsWanted){
    if(bar&&strip.parentNode!==bar){
      if(back&&back.parentNode===bar)bar.insertBefore(strip,back.nextSibling);
