...
RAWNAMES_LOG="$ART_DIR/.raw-names.ds.$MODE.log"
: > "$RAWNAMES_LOG"
...
# Extra exclusions: sprite|filmstrip|_anim|template_skin|blank_icon|macro.
 ...
 *{box-sizing:border-box}
 ...
 .index{column-width:clamp(14rem,28vw,18rem);column-gap:clamp(1rem,2.5vw,2rem);font-size:.9rem;padding-left:1.15rem;list-style:none;--index-name-lines:2}
 .index li{display:flex;align-items:flex-start;gap:.28em;break-inside:avoid;margin:.15rem 0;padding:0;list-style:none;max-width:100%;min-width:0;line-height:1.35;white-space:normal;overflow-wrap:anywhere;word-break:break-word}
 ...
 html[data-ui-scale="110"] .index,html[data-ui-scale="125"] .index,html[data-ui-scale="133"] .index{--index-name-lines:3}
 html[data-ui-scale="140"] .index,html[data-ui-scale="150"] .index,html[data-ui-scale="175"] .index,html[data-ui-scale="200"] .index,html[data-ui-scale="250"] .index,html[data-ui-scale="300"] .index{--index-name-lines:4}
...
cat <<'HTML'
 ...
 .search-chrome{width:100%;max-width:100%;min-width:0;box-sizing:border-box}
 ...
 .search-strip-fs:hover,.search-strip-fs[aria-pressed="true"]{color:var(--accent-instrument);border-color:var(--accent-instrument);background:var(--accent-instrument-bg)}
...
#kwComboSaveGo{width:100%;min-height:2.25rem;border:1px solid var(--accent-instrument);border-radius:6px;background:var(--accent-instrument-bg);color:var(--accent-instrument);font:inherit;cursor:pointer}
 ...
 /* KW FS toggle button in filter-top */
 ...
 /* KW FS back button (always in filter-top, hidden unless kw-fs-open) */
 ...
 .filter-wrap .ac-companion-btn,body.search-mode .ac-companion-btn,body.kw-fs-open .ac-companion-btn{display:inline-flex}
 /* Stripe wrap: the › trigger + sliding panel, sits at right of toolbar */
 ...
 /* Stripe panel: slides LEFT from trigger, position:absolute overlay — no layout shift */
 ...
 /* Snap / pin buttons inside stripe */
 ...
 /* Dual fullscreen: side by side, Search left, KW right */
...
/* AC FS: scrolling down content collapses Row 2 (searchStrip) and stripe trigger */
...
/* KW FS: scrolling down content hides stripe trigger and kw-fs-btn */
...
/* Remove any stale collapsed class when exiting FS mode */
body:not(.ac-fs-open) .search-ac-shell.toolbar-scroll-collapsed>.search-strip{
 ...
 body.display-sides .kw-fs-btn,body.display-sides .search-strip-fs{display:inline-flex!important}
  ...
  body.display-sides #catalogIndex .index li{display:flex!important;align-items:flex-start;gap:.28em;box-sizing:border-box;margin:.12rem 0;padding:0;max-width:100%;min-width:0;min-height:1.4em;line-height:1.35;overflow:hidden;isolation:isolate;position:relative;z-index:0;white-space:normal;overflow-wrap:anywhere;word-break:break-word;list-style:none}
  ...
  body.display-sides #catalogIndex .index a{flex:1 1 auto;min-width:0;max-width:100%;display:block;-webkit-line-clamp:unset;line-clamp:unset;max-height:calc(1.35em * var(--index-name-lines,2));overflow:hidden;white-space:normal;overflow-wrap:anywhere;word-break:break-word;line-height:1.35;vertical-align:top}
...
@media(max-width:899px){
  ...
  body.display-sides #catalogIndex:not(.is-collapsed) #catalogIndexList{display:grid!important;grid-template-columns:repeat(auto-fill,minmax(min(10.5rem,100%),1fr));grid-auto-flow:row;flex:1 1 auto;min-height:0;overflow-y:auto!important;columns:none!important;column-count:1!important;list-style:none;padding:.35rem .6rem .6rem .9rem;gap:.2rem .75rem}
  body.display-sides #catalogIndex .index li{display:flex!important;align-items:flex-start;gap:.28em;min-height:1.35em;line-height:1.35;overflow:visible;white-space:normal;overflow-wrap:anywhere;word-break:break-word}
...
function cardSearchYtPlayUrl(id,host,opts){
  ...
  if(host==='invidious'){
    var instances=cardSearchState._invInstances||['invidious.perennialte.ch','invidious.nerdvpn.de','inv.nadeko.net','yewtu.be','yt.artemislena.eu','vid.puffyan.us'];
    var idx=Number(cardSearchState._invInstanceIdx)||0;
    var inst=instances[idx%instances.length]||'invidious.perennialte.ch';
...
function cardSearchYtIsBadEmbedUrl(url){
  ...
  var isInv=/(?:yewtu\.be|invidious\.nerdvpn\.de|inv\.nadeko\.net|yt\.artemislena\.eu|invidious\.perennialte\.ch|vid\.puffyan\.us|invidious\.[a-z0-9.-]+)\/embed\/[A-Za-z0-9_-]{6,}/.test(s);
...
function setCardYtFrame(id,host,opts){
  ...
  function mountPlain(){
    ...
    if(!f)return;
    if(useInvidious){cardSearchState._invLoadCount=0;var _invFT=Date.now();f.onload=(function(_eu,_ft){return function(){if(cardSearchState.embedUrl!==_eu)return;var _n=(cardSearchState._invLoadCount||0)+1;cardSearchState._invLoadCount=_n;var _ms=Date.now()-_ft;// Fast load < 1500ms on first onload = error page / X-Frame-Options / CF challenge
if(_n===1&&_ms<1500)cardSearchInvTryNext('fast-error');else if(_n>=2&&_ms<8000)cardSearchInvTryNext('cf-loop');};})(url,_invFT);}
    f.src=url;
  }
  function mountApi(){
    try{
...
function cardSearchInvTryNext(reason){
  var instances=cardSearchState._invInstances||[];
  var idx=(cardSearchState._invInstanceIdx||0)+1;
  cardSearchState._invInstanceIdx=idx;
  cardSearchState._invLoadCount=0;
  var id=cardSearchState.ytId||'';
  if(!id){cardSearchOpenPopup();return;}
  if(idx<instances.length){
    if(!setCardYtFrame(id,'invidious',{autoplay:1})){cardSearchOpenPopup();}
  }else{
    // All Invidious instances exhausted.
    // Clear the iframe onload handler BEFORE calling showYtBlocked, which removes
    // the iframe src. Without this, removeAttribute('src') fires another onload on
    // the stale closure, which calls invTryNext again → infinite loop.
    var _fExh=document.getElementById('cardSearchFrame');
    if(_fExh)_fExh.onload=null;
    cardSearchState.embedUrl=''; // invalidate stale-check in any lingering handlers
    cardSearchShowYtBlocked('all-instances-tried');
  }
}
...
function cardSearchShowYtBlocked(reason){
  ...
  if(fallback){
    ...
    if(reason==='all-instances-tried'){
      // All Invidious instances exhausted — don't show play button (would restart the cycle).
      // Show thumbnail (non-interactive) + clear message directing user to Open in popup.
...
window.cardSearchPlayYtFallback=function(){
  ...
  if(!cardSearchYtCanEmbedHere()){
    ...
    // Instances ordered: known non-Cloudflare first, then Cloudflare ones.
    cardSearchState._invInstances=['invidious.perennialte.ch','invidious.nerdvpn.de','inv.nadeko.net','yewtu.be','yt.artemislena.eu','vid.puffyan.us'];
...
function placeAcShell(){
  ...
  if(!acOpen()){
    clearAcShellPos(sh);syncAcWidthBtn();return;}
  var box=viewBox();
  var r=layoutRem();
  if(acFullscreen()){
...
function init(){
  ...
  document.addEventListener('pointerdown',function(e){
    if(!acOpen())return;
    var inside=e.target.closest&&e.target.closest('#acShell,#searchInput,.search-strip,.search-chrome,#filterWrap,.filter-toggle');
    if(inside)return;
    hideSearchAc();
  });
...
function syncDualButtons(){
  ...
  // Show snap+pin only in dual mode
...
function syncKwFsBtn(){
  ...
  var on=kwFsWanted;
  btn.setAttribute('aria-pressed',on?'true':'false');
  btn.setAttribute('aria-label',on?'Exit fullscreen Keywords':'Fullscreen Keywords');
  btn.title=on?'Exit fullscreen':'Fullscreen Keywords';
}
...
function setKwFullscreen(on){
  ...
  if(kwFsWanted){
    ...
    if(w&&!w.classList.contains('open')){
      w.classList.add('open');
      document.body.classList.add('kw-open');
      var arr=w.querySelector('.toggle-arrow');
    ...
    document.body.classList.remove('kw-fs-open');
    // Close any open stripe and reset scroll-collapse
    var panel=document.getElementById('kwFsStripePanel');
    var trig=document.getElementById('kwStripeTrigger');
  ...
  updateDualState();
  syncKwFsBtn();
  // Re-place search ac shell for dual layout
  if(typeof placeAcShell==='function')placeAcShell();
}
...
window.snapDualFsTo50=function(){
  saveRatio(0.5);
  if(dualPinned)savePin(false);
  applyDualLayout();
  syncSepPin();
  // Close stripe after snap
  ['acFsStripePanel','kwFsStripePanel'].forEach(function(id){
    var p=document.getElementById(id);
    if(p)p.classList.remove('stripe-open');
  });
  document.getElementById('acStripeTrigger')&&(document.getElementById('acStripeTrigger').textContent='\u203a');
  document.getElementById('kwStripeTrigger')&&(document.getElementById('kwStripeTrigger').textContent='\u203a');
};
...
function syncNarrowPanels(){
  if(!isDualFs()){
    document.body.classList.remove('dual-ac-narrow','dual-kw-narrow');
    return;
  }
...
(function bindSepDrag(){
  ...
  sep.addEventListener('pointerdown',function(e){
    ...
    active=true;dragId=e.pointerId;
    sep.setPointerCapture(e.pointerId);
    sep.classList.add('sep-dragging');
    e.preventDefault();
    e.stopPropagation();
  });
  sep.addEventListener('pointermove',function(e){
    ...
    var vw=window.innerWidth;
    var minPx=Math.max(160,Math.round(0.15*vw));
    var maxPx=Math.min(vw-160,Math.round(0.85*vw));
  ...
  function onUp(e){
    if(!active||(e&&e.pointerId!=null&&e.pointerId!==dragId))return;
    active=false;
    sep.classList.remove('sep-dragging');
    saveRatio(dualRatio);
  }
  sep.addEventListener('pointerup',onUp);
  sep.addEventListener('pointercancel',onUp);
  sep.addEventListener('lostpointercapture',onUp);
})();

// --- Edge swipe → bring companion in ---
(function bindEdgeSwipe(){
  ...
  document.addEventListener('pointerdown',function(e){
    ...
    if(isAcFs()&&kwFsWanted)return; // already dual
    var vw=window.innerWidth;
  ...
  document.addEventListener('pointerup',function(e){
    ...
    var vw=window.innerWidth;
    var fromLeft=sx<=EDGE,fromRight=(sx>=vw-EDGE);
...
// --- Patch setAcFullscreen to sync dual state ---
(function(){
  var _orig=window.setAcFullscreen;
  if(!_orig)return;
  ...
  var _origToggle=window.toggleAcFullscreen;
  window.toggleAcFullscreen=function(){window.setAcFullscreen(!isAcFs());};
})();

// --- Resize → reapply dual layout ---
